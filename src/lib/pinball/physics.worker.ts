...
function stepPhysics(input: PhysicsInput): PhysicsOutput {
    const { gameSpeed: speed, paused, gameState } = input

    const ball: BallState = {
        ...input.ball,
        position: { ...input.ball.position },
        velocity: { ...input.ball.velocity },
    }
    const flippers: FlipperState[] = input.flippers.map((f) => ({
        ...f,
        pivot: { ...f.pivot },
    }))
    const bumpers: BumperState[] = input.bumpers.map((b) => ({
        ...b,
        position: { ...b.position },
    }))
    const targets: TargetState[] = input.targets.map((t) => ({
        ...t,
        position: { ...t.position },
    }))
    const posts: PostState[] = input.posts.map((p) => ({
        ...p,
        position: { ...p.position },
    }))
    ...
    const launcher: LauncherState = {
        ...input.launcher,
        position: { ...input.launcher.position },
    }
    ...
    for (const b of bumpers) {
        if (b.hitAnimation > 0)
            b.hitAnimation = Math.max(0, b.hitAnimation - 0.08 * speed)
    }
    for (const t of targets) {
        if (t.hitAnimation > 0)
            t.hitAnimation = Math.max(0, t.hitAnimation - 0.04 * speed)
    }
    for (const s of slingshots) {
        if (s.hitAnimation > 0)
            s.hitAnimation = Math.max(0, s.hitAnimation - 0.1 * speed)
    }
    for (const p of posts) {
        if (p.hitAnimation > 0)
            p.hitAnimation = Math.max(0, p.hitAnimation - 0.12 * speed)
    }
    ...
    for (let step = 0; step < SUBSTEPS; step++) {
        ...
        for (const rail of input.guideRails) {
            ...
            if (c.collided) {
                const newPos = new Vector2D(bx, by).add(
                    c.normal.multiply(c.overlap + 0.5)
                )
                bx = newPos.x
                by = newPos.y
                const ref = reflectVelocity(
        ...
        for (const t of targets) {
            ...
            const cx = Math.max(
                t.position.x - halfW,
                Math.min(bx, t.position.x + halfW)
            )
            const cy = Math.max(
                t.position.y - halfH,
                Math.min(by, t.position.y + halfH)
            )
            const dx = bx - cx
            const dy = by - cy
        ...
        for (const s of slingshots) {
            ...
            for (const [start, end] of walls) {
                ...
                if (c.collided) {
                    ...
                    s.hitAnimation = 1
                    const scx =
                        (s.vertices[0].x + s.vertices[1].x + s.vertices[2].x) /
                        3
                    const scy =
                        (s.vertices[0].y + s.vertices[1].y + s.vertices[2].y) /
                        3
                    hits.push({
                        points: s.points,
                        x: scx,
